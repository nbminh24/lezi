...
import { ArrowLeft, Check, SpeakerHigh } from 'phosphor-react';
import FeedbackSheet from '../FeedbackSheet';

const ListeningScreen = ({ onBack, onComplete }) => {
    const [selectedId, setSelectedId] = useState(null);
    const [status, setStatus] = useState('idle'); // 'idle', 'correct', 'wrong'
    const [isPlaying, setIsPlaying] = useState(false);

    // Mock data
    const audioText = "The bear likes honey";
    const options = [
        { id: 1, text: 'Con gấu thích mật ong', isCorrect: true },
        { id: 2, text: 'Con ong thích gấu', isCorrect: false },
        { id: 3, text: 'Mật ong rất ngọt', isCorrect: false },
        { id: 4, text: 'Con gấu đang ngủ', isCorrect: false },
    ];
    ...
    const playAudio = () => {
        console.log(`🔊 Playing audio: "${audioText}"`);
        setIsPlaying(true);
    ...
    return (
        <div className="relative min-h-screen bg-[#F9FAFB] flex flex-col">
            ...
            <div className="flex-1 px-6 pt-[90px] pb-32 overflow-y-auto">
                    ...
                    <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1 bg-white rounded-3xl rounded-tl-sm px-6 py-4 shadow-[0_4px_12px_rgba(0,0,0,0.08)]">
                            <p className="text-[18px] font-semibold text-[#1a1a1a]">
                                <span className="text-blue-600 font-bold">Tap to listen</span>, then choose the meaning!
                            </p>
                        </div>
                    </div>
                </div>

                {/* Giant Speaker Button (Hero) */}
              ...
              border-b-[8px] border-b-blue-700
              shadow-xl shadow-blue-500/30
              flex items-center justify-center
              transition-all duration-150
              ${isPlaying ? 'cursor-wait' : 'cursor-pointer hover:scale-105'}
              active:border-b-0 active:shadow-lg active:translate-y-[8px]
            `}
                    >
                ...
                <div className="flex flex-col gap-3">
                  ...
                  ${isSelected
                                        ? 'bg-blue-50 border-2 border-blue-400 border-b-[4px] border-b-blue-500 text-blue-600 shadow-[0_4px_0_0_#3b82f6] active:border-b-0 active:shadow-none active:translate-y-[4px]'
                                        : 'bg-white border-2 border-gray-100 border-b-[4px] border-b-gray-200 text-gray-700 shadow-[0_4px_0_0_#e5e7eb] hover:border-gray-200 active:border-b-0 active:shadow-none active:translate-y-[4px]'
                                    }
          ...
          `}
            ...
            <FeedbackSheet
                status={status}
                message={status === 'correct' ? 'Great listening!' : null}
